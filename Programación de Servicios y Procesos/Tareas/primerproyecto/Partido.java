public class Partido {
	...
	private void jugadaLocal() {
		...
		for (Jugador jugador : local.getConvocatoria()) {
			if (tiro(jugador.getTiro()) && !parada(visitante)) {
				golesLocal++;
				System.out.println(jugador.getNombre() + " " + minuto + "\'" + (isPenalti() ? " (P)" : "") + " (" + local.getNombre() + ")");
				espera();
				break;
			}
		}
	...
	private void jugadaVisitante() {
		...
		for (Jugador jugador : visitante.getConvocatoria()) {
			if (tiro(jugador.getTiro()) && !parada(local)) {
				golesVisitante++;
				System.out.println(jugador.getNombre() + " " + minuto + "\'" + (isPenalti() ? " (P)" : "") + " (" + visitante.getNombre() + ")");
				espera();
				break;
			}
		}
	...
	private void penaltis(boolean moneda) {
		System.out.println("\n" + local.getNombre() + " " + golesLocal + ((penaltisLocal != 0) ? "(" + penaltisLocal + ") - " : " - ") + golesVisitante + ((penaltisVisitante != 0) ? "(" + penaltisVisitante + ") - " : " ") + visitante.getNombre());
		...
		for (int i = 0; i < PENALTIS; i++) {
			...
			if ((PENALTIS - 1) - i + penaltisLocal < penaltisVisitante || (PENALTIS - 1) - i + penaltisVisitante < penaltisLocal) {
				break;
			}
		...
		if (penaltisLocal != penaltisVisitante) {
			esperaLenta();
			System.out.println("\n---------------- RESULTADO FINAL ----------------\n" + local.getNombre() + " " + golesLocal + "(" + penaltisLocal + ") - " + golesVisitante + "(" + penaltisVisitante + ") " + visitante.getNombre());
		} else {
			penaltis(moneda);
		}
	...
	private void penaltiLocal(int tirador) {
		...
		if (penalti(local.getConvocatoria()[tirador].getTiro()) && !paradaPenalti(visitante)) {
			penaltisLocal++;
			System.out.println("Goool de " + local.getConvocatoria()[tirador].getNombre() + " (" + local.getNombre() + ")");
		} else {
			System.out.println("Fallo de " + local.getConvocatoria()[tirador].getNombre() + " (" + local.getNombre() + ")");
		}
	...
	private void penaltiVisitante(int tirador) {
		...
		if (penalti(visitante.getConvocatoria()[tirador].getTiro()) && !paradaPenalti(local)) {
			penaltisVisitante++;
			System.out.println("Goool de " + visitante.getConvocatoria()[tirador].getNombre() + " (" + visitante.getNombre() + ")");
		} else {
			System.out.println("Fallo de " + visitante.getConvocatoria()[tirador].getNombre() + " (" + visitante.getNombre() + ")");
		}
	...
	private boolean parada(Equipo equipo) {
		...
		if (aleatoriedad()) {
			...
			parada = (int)(Math.random() * 100) <= equipo.getConvocatoria()[(int)(Math.random() * equipo.getConvocatoria().length)].getParada();
		}
