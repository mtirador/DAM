...
19
//ALBERTO
SELECT DATE_FORMAT(NOW(),'HOY ES: %W');

//ME
SELECT DATE_FORMAT(ADDDATE(CURDATE(), INTERVAL 3 MONTH), '%d-%m-%Y') AS fecha, 
ELT(WEEKDAY(ADDDATE(CURDATE(), INTERVAL 3 MONTH))+1, 'LUNES', 'MARTES', 'MIERCOLES', 'JUEVES', 'VIERNES', 'SABADO', 'DOMINGO') AS dia_semana;
...
SELECT DATEDIFF(FFINAL,FINICIAL)DIAS,
INTERVAL(DATEDIFF(FFINAL,FINICIAL),0,3,6,11);
...
SELECT MARCA, GROUP_CONCAT(DISTINCT MODELO SEPARATOR '----')
FROM AUTOMOVILES
GROUP BY MARCA;
...
22. Crea una tabla con una sola columna que contenga el valor encriptado de los
nombres y apellidos de los clientes mediante alguno de los sistemas de encriptación
usando la clave FEBRERO. Comprueba que se desencripta correctamente.


CREATE TABLE EJEMPLO
SELECT ENCODE(CONCAT(NOMBRE,' ',APELLIDOS),'FEBRERO')AS NOMBRE
FROM CLIENTES;

CREATE TABLE ejemplo
Select encode(CONCAT(nombre,' ',apellidos),'Febrero')
AS nombre
from clientes;

SELECT DECODE(NOMBRE,'FEBRERO')FROM CLIENTES;
